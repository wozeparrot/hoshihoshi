            ...
            if lmks is not None:
                ...
                if self.head_rotation[1] > 15:
                    ...
                    self.left_iris_ratio = lerp(
                        0.4, self.left_iris_ratio, self.right_iris_ratio
                    )
                    self.right_iris_ratio = lerp(
                        0.4, self.right_iris_ratio, old_left_iris_ratio
                    )
...
def main(args) -> None:
    ...
    proc = ThreadedProcessing(cap_queue, out_queue, head_rotation_offsets=(12, 5, 0.12))
    proc.start()
